...
-------------------------------------------------------------------------
	...
	ang1:RotateAroundAxis( oang1:Up(), -180 )
	ang1:RotateAroundAxis( oang1:Right(), 0 )
	ang1:RotateAroundAxis( oang1:Up(), 0)

	pos1 = pos1 + oang1:Forward()*-2 + oang1:Up() * 2.5 + oang1:Right() * 0
		
		plate:SetModel("models/props_combine/combine_light002a.mdl")
		plate:SetPos( pos1 )
		plate:SetAngles( ang1 )
		plate:SetCollisionGroup( COLLISION_GROUP_IN_VEHICLE )
		plate:SetModelScale( 0.15 )
...
function ENT:Use(activator, player)
	...
	timer.Simple(3, function()
		...
		player:Give("swep_ar_health")
		player:EmitSound( "items/smallmedkit1.wav")
		player:ChatPrint("[ar]You have now installed the Anderson Robotics Health Regeneration Augment!")
